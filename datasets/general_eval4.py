...
class MVSDataset(Dataset):
        ...
        for scan in scans:
            ...
            with open(os.path.join(self.datapath, pair_file)) as f:
                ...
                for view_idx in range(num_viewpoint):
                    valid_list = list(range(0,49,sparsity))
                    ...
                    if len(src_views) > 0:
                        ...
                        src_views_ = src_views.copy()
                        src_views = [item for item in src_views_ if item%sparsity == 0]                   

                        src_no_views = [(item - item%sparsity) if (item%sparsity) <= sparsity/2 or item + item%sparsity > 48 else (item + (sparsity-item%sparsity)) for item in src_views_ if item%sparsity != 0]
                        
                        filtered_src_views_ = src_views + src_no_views     
                        filtered_src_views_ = list(dict.fromkeys(filtered_src_views_))
                        
                        if ref_view%sparsity == 0:
                            filtered_src_views = [item for item in filtered_src_views_ if item != ref_view]
                            
                            no_src_valid = [item for item in valid_list if not (item in filtered_src_views or item == ref_view)]
                            no_src_valid = sorted(no_src_valid, key=lambda x: abs(x - ref_view))

                            metas.append((scan, ref_view, filtered_src_views+no_src_valid, scan))
                        else:
                            continue
    ...
    def crop_mvs_input(self, img, intrinsics, max_w, max_h, base=64):
        h, w = img.shape[:2]
        if h > max_h or w > max_w:
            scale = 1.0 * max_h / h
            if scale * w > max_w:
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            img, intrinsics = self.crop_mvs_input(img, intrinsics, self.max_w, self.max_h)
