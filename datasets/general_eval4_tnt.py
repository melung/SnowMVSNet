...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, interval_scale=1.06, **kwargs):
        ...
        self.fix_wh = False
        #self.split = 'advanced'
        self.split = 'intermediate'
        #'Horse': 1.5e-3 'Panther': 5e-3 'M60': 5e-3' Playground': 7e-3
        self.depth_interval_table = {
            # intermediate
            'Family': 2.5e-3, 'Francis': 1e-2, 'Horse': 2.0e-3, 'Lighthouse': 1.5e-2, 'M60': 5e-3, 'Panther': 5e-3, 'Playground': 7e-3, 'Train': 5e-3, 
            # advanced
    ...
    def read_cam_file(self, filename, interval_scale):
        ...
        depth_interval = float(lines[11].split()[-1]) #depth max
    ...
    def crop_mvs_input(self, img, intrinsics, max_w, max_h, base=64):
        ...
        img: W H C
        """
        intrinsics[1,2] =  intrinsics[1,2] - 28.0/4  # 1080 -> 1024
        img = img[28:1080-28, :, :]
        return img, intrinsics
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                depth_max = depth_min + self.ndepths * self.depth_interval_table[scene_name]
