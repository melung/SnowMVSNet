...
def generate_points_from_depth(depth, proj, intra_w):
    '''
    :param depth: (B, 1, H, W)
    :param proj: (B, 4, 4)
    ...
    proj_xyz = torch.cat((proj_xyz,intra_w),dim = 1) # [B, 4, H, W]

    #print('proj_xyz', proj_xyz.size())
    #print('intra', intra_w.size())
    return proj_xyz
...
def filter_depth(ref_depth, src_depths, ref_proj, src_projs, ref_intra_w, src_intra_ws):
    '''
    ...
    ref_pc = generate_points_from_depth(ref_depth, ref_proj, ref_intra_w)
    src_pcs = generate_points_from_depth(src_depths, src_projs, src_intra_ws)

    aligned_pcs = homo_warping(src_pcs, src_projs, ref_proj, ref_depth)
    #print('aligned_pcs',aligned_pcs.size())
...
def load_data(root_path, depth_path, scene_name, thresh):
    ...
    for view in range(49):
        ...
        intra_w_filename = "{}/{}/intra_w/{:08d}.pfm".format(depth_path, scene_name, view)
        ...
        intra_w_map, _ = read_pfm(intra_w_filename)
        intra_w_map = cv2.resize(intra_w_map, (w, h), interpolation=cv2.INTER_LINEAR)

        
        dep_map = dep_map * (conf_map>thresh).astype(np.float32)
        depths.append(torch.from_numpy(dep_map).unsqueeze(0))

        intra_w.append(torch.from_numpy(intra_w_map).unsqueeze(0))

        rgb = np.array(Image.open(img_filename))
        rgbs.append(rgb)

    intra_ws = torch.stack(intra_w).float()
    depths = torch.stack(depths).float()
    projs = torch.stack(projs).float()
    if args.device == 'cuda' and torch.cuda.is_available():
        depths = depths.cuda()
        projs = projs.cuda()
        intra_ws = intra_ws.cuda()
    return depths, projs, rgbs, intra_ws
...
def open3d_filter():
    with torch.no_grad():
        ...
        for i, scene in enumerate(all_scenes):
            ...
            depths, projs, rgbs, intra_ws = load_data(args.root_path, args.depth_path, scene, args.prob_thresh)
            ...
            for i in range(tot_frame):
                ...
                intra_w_sum = torch.zeros((1, height, width), device=depths.device, dtype=depths.dtype)
                ...
                while True:
                    ref_pc, pcs, dist = filter_depth(ref_depth=depths[i:i+1], src_depths=depths[j:min(j+batch_size, tot_frame)],
                                                    ref_proj=projs[i:i+1], src_projs=projs[j:min(j+batch_size, tot_frame)], ref_intra_w = intra_ws[i:i+1], src_intra_ws = intra_ws[j:min(j+batch_size, tot_frame)])
                    #print("pcs",pcs.size())
                    masks = (dist < args.dist_thresh).float()
                    #print('mask',masks.size())
                    masked_pc = pcs[:,:,:,:] * masks
                    masked_intra_w = masked_pc[:,3:4,:,:]
                    masked_pc = masked_pc[:,:3,:,:]
                    
                    #print('mask pc', masked_pc[0,:,500,500])
                    #print('intra mask pc', (masked_intra_w*masked_pc)[0,:,500,500])
                    
                    
                    pc_buff += (masked_pc).sum(dim=0, keepdim=False)
                    val_cnt += masks.sum(dim=0, keepdim=False)
                    intra_w_sum += masked_intra_w.sum(dim = 0, keepdim=False)
                ...
                final_mask = (intra_w_sum  >= args.num_consist - 0.5 ).squeeze(0)
