...
parser.add_argument("--root_dir", type=str, default="/hdd1/lhs/dev/code/MVSTER/outputs/tnt/8844/intermediate/")
...
parser.add_argument('--dist_base', type=float, default= 0.5*(1 / 4))
parser.add_argument('--rel_diff_base', type=float, default= 0.5*(1 / 1300))
...
def filter_depth(scene, root_dir, split, out_dir, plyfilename, fusion_exp):
    ...
    for ref_view, src_views in pair_data:
        ...
        #ref_depth_est[ref_depth_est>3] = 0
        ref_depth_est[ref_depth_est<0] = 0
        ...
        for src_view in src_views:
            ...
            #print(src_depth_est>0)
            #src_depth_est[src_depth_est>10] = 0
            src_depth_est[src_depth_est<0] = 0
...
def dypcd_filter_worker(scene):
    ...
    filter_depth(scene, args.root_dir, args.split, args.out_dir, os.path.join(args.ply_path, save_name), fusion_exp)
...
if __name__ == '__main__':
    ...
    for fusion_exp in tnt_fusion_exps:
        ...
        if not os.path.isdir(os.path.join(args.ply_path)):
            os.mkdir(os.path.join(args.ply_path))
