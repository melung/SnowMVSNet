        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    print(
                       "Epoch {}/{}, Iter {}/{}, lr {:.6f}, train loss = {:.3f}, d_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, c_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, range_err = {:.3f}, {:.3f}, {:.3f}, {:.3f}, n_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, time = {:.3f}".format(
                           ...
                           scalar_outputs["s0_n_loss"],
                           scalar_outputs["s1_n_loss"],
                           scalar_outputs["s2_n_loss"],
                           scalar_outputs["s3_n_loss"],
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print(
                            "Epoch {}/{}, Iter {}/{}, lr {:.6f}, test loss = {:.3f}, d_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, c_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, range_err = {:.3f}, {:.3f}, {:.3f}, {:.3f}, n_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, time = {:.3f}".format(
                            epoch_idx, args.epochs, batch_idx, len(TrainImgLoader),
                               optimizer.param_groups[0]["lr"], 
                           loss,
                           scalar_outputs["s0_d_loss"],
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    outputs = model(sample_cuda["imgs"], sample_cuda["R"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]

    loss, stage_d_loss, stage_c_loss, range_err_ratio, stage_normal_loss = model_loss(sample_cuda["depth_values"],
    ...
    scalar_outputs = {"loss": loss,
                      ...
                      "s0_n_loss": stage_normal_loss[0],
                      "s1_n_loss": stage_normal_loss[1],
                      "s2_n_loss": stage_normal_loss[2],
                      "s3_n_loss": stage_normal_loss[3],
...
def test_sample_depth(model, model_loss, sample, args):
    ...
    outputs = model_eval(sample_cuda["imgs"], sample_cuda["R"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
    ...
    scalar_outputs = {"loss": loss,
                      ...
                      "filtered_abs_depth_error": AbsDepthError_metrics(depth_est, depth_gt, fmask),
                      "filtered_thres2mm_error": Thres_metrics(depth_est, depth_gt, fmask, 2),
                    #   "upbo_abs_depth_error": AbsDepthError_metrics(depth_reg, depth_gt, mask),
                    #   "upbo_thres2mm_error": Thres_metrics(depth_reg, depth_gt, mask, 2)
                    }
